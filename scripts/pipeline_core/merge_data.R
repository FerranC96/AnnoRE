...
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
#Merge data and generate main_output
#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~#
...
main_haploR <- subset(main_haploR, select = c(rsID, DNAse, Proteins, eQTL,
...
vep_output <- vep_output %>% dplyr::mutate(rsID = str_extract(Existing_variation,
                                                  pattern = "rs\\d+(?=\\,|$)"))

merged <- left_join(vep_output, main_haploR, by=c("rsID"))
merged <- merged %>% dplyr::mutate(chr = str_extract(Location,
                                                      pattern = "^.*(?=\\:)"))

write.table(merged, file="./output/OCD_main_output.tsv", quote=FALSE,
...
chr_distro <- feature_type <- merged %>% group_by(chr) %>% 
...
feature_type <- merged %>% group_by(Feature_type) %>%
                  summarise(Counts = n_distinct(X.Uploaded_variation))
feature_type_filtered <- feature_type[!(feature_type$Feature_type == "-"),] %>%
                          mutate(percentage=Counts/sum(Counts)*100.0)
write.table(feature_type_filtered, file="./output/feature_type.tsv",
...
regulatory_subset <- subset(regulatory, select = c(
                              X.Uploaded_variation, Location, Feature, BIOTYPE,
                              DNAse, Promoter_histone_marks, Enhancer_histone_marks))
write.table(regulatory_subset, file = "./output/regulatory_variants.tsv",
            quote = FALSE, sep="\t")

#Biotype
reg_biotype <- regulatory_subset %>%  group_by(BIOTYPE) %>%
                summarise(counts = n_distinct(X.Uploaded_variation))
write.table(reg_biotype, file="./output/reg_bio.tsv", quote=FALSE,
            row.names = FALSE, sep="\t")
...
reg_phistone <- separate_rows(regulatory_subset, Promoter_histone_marks) %>%
                group_by(Promoter_histone_marks) %>% summarise(
                                    counts = n_distinct(X.Uploaded_variation))
reg_phistone_filtered <- reg_phistone[(!(reg_phistone$Promoter_histone_marks=="NA")
                          & !(reg_phistone$Promoter_histone_marks=="")),] %>% drop_na()
write.table(reg_phistone_filtered, file="./output/reg_phist.tsv", quote=FALSE,
            row.names = FALSE, sep="\t")

reg_phist_plot <- ggplot(reg_phistone_filtered ,aes(x = Promoter_histone_marks,
  y = counts, fill = Promoter_histone_marks)) +
  geom_bar(stat = "identity") +
  theme_classic() + theme(plot.title=element_text(face="bold")) + 
...
reg_ehistone <- separate_rows(regulatory_subset, Enhancer_histone_marks) %>% 
                group_by(Enhancer_histone_marks) %>% summarise(
                                    counts = n_distinct(X.Uploaded_variation))
reg_ehistone_filtered <- reg_ehistone[(!(reg_ehistone$Enhancer_histone_marks=="NA")
                          & !(reg_ehistone$Enhancer_histone_marks=="")),] %>% drop_na()
write.table(reg_ehistone_filtered, file="./output/reg_ehist.tsv", quote=FALSE, row.names = FALSE, sep="\t")

reg_ehist_plot <- ggplot(reg_ehistone_filtered ,aes(x = Enhancer_histone_marks,
  y = counts, fill = Enhancer_histone_marks)) +
  geom_bar(stat = "identity") +
  theme_classic() + theme(plot.title=element_text(face="bold")) +
  theme(axis.text.x = element_text(angle=45, vjust=1, hjust=1)) +
  labs(
...
#DNase 
reg_dnase <- separate_rows(regulatory_subset, DNAse) %>% group_by(DNAse) %>% 
              summarise(counts = n_distinct(X.Uploaded_variation))
reg_dnase_filtered <- reg_dnase[(!(reg_dnase$DNAse=="NA") & !(reg_dnase$DNAse=="")),] %>% drop_na()
write.table(reg_dnase_filtered, file="./output/reg_dnase.tsv", quote=FALSE,
            row.names = FALSE, sep="\t")
